...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'generate_sitemap') {
        if ($sitemapExists) {
            $_SESSION['flash'] = ['type' => 'warning', 'message' => 'Sitemap already generated. Delete the existing sitemap first to regenerate.'];
        } else {
    ...
    } elseif ($action === 'delete_sitemap') {
        if ($sitemapExists && unlink($sitemapPath)) {
            $_SESSION['flash'] = ['type' => 'success', 'message' => 'Sitemap deleted successfully'];
            $sitemapExists = false;
        } else {
            $_SESSION['flash'] = ['type' => 'danger', 'message' => 'Failed to delete sitemap'];
        }
    }
                ...
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    ...
                    <div>
                        <form method="POST" action="" class="d-inline">
                            ...
                            <button type="submit" class="btn btn-primary btn-sm"><i class="fas fa-plus mr-1"></i> Generate Sitemap</button>
                        </form>
                        <?php if ($sitemapExists): ?>
                        <form method="POST" action="" class="d-inline" onsubmit="return confirm('Delete sitemap.xml?');">
                            <input type="hidden" name="action" value="delete_sitemap">
                            <button type="submit" class="btn btn-danger btn-sm ml-1"><i class="fas fa-trash mr-1"></i> Delete Sitemap</button>
                        </form>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-lg-8">
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <div class="sitemap-status">
                                    <?php if ($sitemapExists): ?>
                                        <p class="text-success"><i class="fas fa-check-circle mr-2"></i>Sitemap file <strong>sitemap.xml</strong> exists.</p>
                                        <p><i class="fas fa-file-code mr-2"></i>Size: <strong><?= number_format(filesize($sitemapPath)) ?> bytes</strong></p>
                                        <p><i class="fas fa-calendar-alt mr-2"></i>Last modified: <strong><?= date('F d, Y h:i A', filemtime($sitemapPath)) ?></strong></p>
                                    ...
                                    <?php else: ?>
                                        <p class="text-muted"><i class="fas fa-exclamation-circle mr-2"></i>Sitemap file <strong>sitemap.xml</strong> does not exist.</p>
                                        <p class="text-muted">Click <strong>"Generate Sitemap"</strong> to create it.</p>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card mb-4">
                            <div class="card-header py-3 d-flex justify-content-between align-items-center">
                                <h6 class="m-0 font-weight-bold">Sitemap Preview</h6>
                                <?php if ($sitemapExists): ?>
                            ...
                            <div class="card-body">
                                <?php if ($sitemapExists): ?>
                                    <div class="sitemap-preview"><?= htmlspecialchars(file_get_contents($sitemapPath)) ?></div>
                                <?php else: ?>
                                    <p class="text-muted text-center my-4">No sitemap to preview. Generate one first.</p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <p><i class="fas fa-info-circle mr-2 text-primary"></i> The sitemap includes:</p>
                                <ul class="list-unstyled ml-4">
                                    <li><i class="fas fa-globe mr-2 text-success"></i> Static pages</li>
                                ...
                                <hr>
                                <p class="mb-0 text-muted small"><i class="fas fa-lightbulb mr-1"></i> Submit sitemap URL to Google Search Console for better indexing.</p>
                            </div>
                        </div>
                    </div>
                </div>
